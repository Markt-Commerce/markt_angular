            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div class="space-y-4">
                <div class="relative">
                  ...
                  <div
                    class="absolute bottom-4 left-4 bg-black bg-opacity-60 text-white px-2 py-1 rounded text-sm"
                  >
              ...
              <div class="space-y-6">
                <div>
                  ...
                  <div class="flex items-center space-x-4 mb-4">
                    ...
                    <span class="text-muted text-sm"
                      >{{ product.sold_count || 387 }} views</span
                    >
                  </div>
                ...
                <div
                  ...
                  *ngIf="product.details && product.details.length > 0"
                >
                ...
                <div class="flex-1">
                  <div class="flex items-center space-x-2">
                    <h3 class="font-semibold text-dark">
                      {{ product.seller?.shop_name || 'Alex Chen' }}
                    </h3>
                    <fa-icon
                      [icon]="faCheckCircle"
                  ...
                  <div class="flex items-center space-x-2">
                    ...
                    <span class="text-muted text-sm"
                      >{{ product.seller?.rating || 4.9 }} ({{
                        product.seller?.review_count || 47
                      }}
                      reviews)</span
                    >
                  </div>
              ...
              <div class="space-y-3 mb-6 text-sm">
                ...
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-dark"
                    >{{ product.seller?.items_sold || 23 }} items</span
                  >
                </div>
              </div>
              ...
              <div class="space-y-3">
                <div class="flex items-center space-x-3">
                  ...
                  <span class="text-sm text-dark"
                    >Verified Stanford student</span
                  >
                </div>
                <div class="flex items-center space-x-3">
                ...
                >
                  ...
                  <div class="flex-1">
                    ...
                    <p class="text-sm font-semibold text-primary">
                      {{ relatedProduct.price | currency : 'USD' }}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div class="text-center">
              ...
              <div class="text-muted text-sm">
                Based on {{ product.review_count || 124 }} reviews
              </div>
            </div>
            <div class="space-y-2">
              <div class="flex items-center space-x-2">
            ...
            <div class="space-y-2">
              <button
                class="w-full text-left px-3 py-2 border border-border rounded-lg hover:bg-light transition-colors text-sm"
              >
                Most Recent
              </button>
              <button
                ...
                <div class="flex-1">
                  <div class="flex items-center space-x-2 mb-2">
                    <h4 class="font-medium text-dark">
                      {{ review.user?.username || 'Sarah Johnson' }}
                    </h4>
                    <div class="flex text-yellow-400 text-sm">
                      <fa-icon
                        [icon]="faStar"
                  ...
                  <p class="text-muted mb-3">
                    {{
                      review.content ||
                        'Excellent condition as described! Alex was very responsive and the pickup was smooth. The laptop works perfectly for my coursework. Highly recommend!'
                    }}
                  </p>
                  <div class="flex space-x-2" *ngIf="review.images?.length">
                    <img
                      ...
                      [src]="img"
                      [alt]="'Review photo'"
                      *ngFor="let img of review.images.slice(0, 2)"
                    />
                  </div>
                </div>
              </div>
...
export class ProductDetailComponent implements OnInit {
  ...
  private loadProduct(): void {
    ...
    if (productId) {
      ...
      this.marketplaceService
        ...
        .subscribe({
          next: (product) => {
            // Convert domain Product to component format (domain model may have different structure)
    ...
    this.marketplaceService.getProducts(params).subscribe({
      next: (products) => {
        ...
        this.relatedProducts = products.map((p) =>
          this.convertDomainProductToComponentFormat(p)
        );
      },
      error: (error) => {
   ...
   *
   * The domain Product model only contains core business properties (id, name, price, stock, etc.)
   * but the component needs full product data (images, description, seller, etc.).
   *
   * This method handles both cases:
   * 1. If the product already has full data (from API/DTO), return as-is
   * 2. If it's a domain model, map what's available and leave rest as undefined
   *    (component handles missing properties with optional chaining)
   */
  private convertDomainProductToComponentFormat(domainProduct: any): any {
    ...
    // This handles cases where API returns full data or mock data
    if (
      domainProduct &&
      (domainProduct.images ||
        domainProduct.description ||
        domainProduct.seller)
    ) {
      return domainProduct;
    }

    // Convert domain Product model to component format
    // Domain Product model has: id, name, price (via getPrice()), stock (via getStock()),
    // status, sellerId, categoryIds, averageRating, reviewCount, createdAt, updatedAt
    // Note: Domain model doesn't have images, description, seller - these will be undefined
    // Component handles missing properties with optional chaining and fallbacks
    return {
      id: domainProduct.id,
      name: domainProduct.name,
      description: domainProduct.description || '', // Domain model doesn't have this
      price:
        typeof domainProduct.getPrice === 'function'
          ? domainProduct.getPrice()
          : domainProduct.price || 0,
      compare_at_price: domainProduct.compare_at_price,
      stock:
        typeof domainProduct.getStock === 'function'
      ...
      category_ids:
        domainProduct.categoryIds || domainProduct.category_ids || [],
      category_id:
        domainProduct.categoryIds?.[0] ||
        domainProduct.category_ids?.[0] ||
        domainProduct.category_id,
      tag_ids: domainProduct.tag_ids || [],
      media_ids: domainProduct.media_ids || [],
      variants: domainProduct.variants || [],
      images: domainProduct.images || [], // Domain model doesn't have this - will be empty
      seller: domainProduct.seller || {}, // Domain model doesn't have this - will be empty
      average_rating:
        domainProduct.averageRating || domainProduct.average_rating || 0,
      rating: domainProduct.averageRating || domainProduct.average_rating || 0,
      review_count:
        domainProduct.reviewCount || domainProduct.review_count || 0,
      ...
      // Preserve any additional properties that might exist on the object
      ...Object.fromEntries(
        Object.entries(domainProduct).filter(
          ([key]) =>
            ![
              'id',
              'name',
    ...
    this.cartService.addToCart(this.product.id, this.quantity).subscribe({
      next: () => {
        ...
        this.router.navigate([ROUTES_ABSOLUTE.APP.CART], {
          queryParams: { source: 'product', productId: this.product.id },
        });
      },
      error: (error) => {
    ...
    this.cartService.addToCart(this.product.id, this.quantity).subscribe({
      next: () => {
        ...
        this.router.navigate([ROUTES_ABSOLUTE.APP.CHECKOUT], {
          queryParams: { source: 'buynow', productId: this.product.id },
        });
      },
      error: (error) => {
        console.error('Error adding to cart:', error);
  ...
  makeOffer(): void {
    ...
    this.router.navigate([
      `${ROUTES_ABSOLUTE.APP.OFFERS.ROOT}/make`,
      this.product.id,
    ]);
  }
  ...
  private loadMockProduct(productId: string): void {
    ...
    const mockProducts: { [key: string]: any } = {
      'macbook-pro-13-2021': {
        ...
        condition: 'Like New',
        description:
          'Excellent condition MacBook Pro 13" 2021 with M1 chip. Barely used, comes with original charger and box. Perfect for students and professionals.',
        details: [
        ...
        images: [
          {
            media: {
              desktop_url:
                'https://storage.googleapis.com/uxpilot-auth.appspot.com/9e580a234a-dfedd1ac85a893bb0f65.png',
            },
          },
          {
            media: {
              desktop_url:
                'https://storage.googleapis.com/uxpilot-auth.appspot.com/d2e5820307-936957b5d254a5a5cb38.png',
            },
          },
          {
            media: {
              desktop_url:
                'https://storage.googleapis.com/uxpilot-auth.appspot.com/1d0a014a05-98e516dc00329c7cf656.png',
            },
          },
        ],
        seller: {
        ...
        trust_indicators: [
          {
            ...
            description: 'Full refund if not as described',
          },
          {
            icon: 'faTruck',
            text: 'Fast Shipping',
            description: 'Free shipping within 2-3 days',
          },
          {
            icon: 'faCheckCircle',
            text: 'Verified Seller',
            description: 'Identity and payment verified',
          },
        ],
      },
      'calculus-textbook-bundle': {
        ...
        condition: 'Good',
        description:
          'Complete calculus textbook bundle for Math 101-102. Includes Stewart Calculus 8th Edition and practice workbook. Some highlighting but all pages intact.',
        details: [
        ...
        trust_indicators: [
          ...
          },
          {
            icon: 'faTruck',
            text: 'Fast Shipping',
            description: 'Free shipping within 1-2 days',
          },
          {
            icon: 'faCheckCircle',
            text: 'Verified Seller',
      ...
      'vintage-denim-jacket': {
        ...
        condition: 'Very Good',
        description:
          'Authentic vintage denim jacket from the 90s. Size M, perfect for campus style. Light wear but no tears or stains. Great for layering.',
        details: [
        ...
        trust_indicators: [
          ...
          },
          {
            icon: 'faCheckCircle',
            text: 'Verified Seller',
            description: 'Fashion enthusiast verified',
          },
        ],
      },
      'study-desk-with-drawers': {
        ...
        condition: 'Excellent',
        description:
          'Perfect study desk for dorm room setup. Includes 3 drawers for storage. Lightweight and easy to assemble. Barely used, like new condition.',
        details: [
        ...
        images: [
          {
            media: {
              desktop_url:
                'https://storage.googleapis.com/uxpilot-auth.appspot.com/92f2ea4247-12d5dc88e2f1267dcf65.png',
            },
          },
        ],
        seller: {
        ...
        trust_indicators: [
          ...
          },
          {
            icon: 'faCheckCircle',
            text: 'Verified Seller',
            description: 'Furniture seller verified',
          },
        ],
      },
    };
    ...
    this.reviews = [
      {
        ...
        rating: 5,
        comment:
          'Exactly as described! Fast shipping and great communication. Highly recommend this seller.',
      ...
      },
      {
        ...
        rating: 4,
        comment:
          'Good quality product. Minor wear as expected for the condition. Seller was very responsive.',
      ...
      },
      {
        ...
        rating: 5,
        comment:
          'Perfect! Better than expected. Will definitely buy from this seller again.',
