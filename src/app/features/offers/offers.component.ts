...
import { ApiService } from '../../core/services/api.service'; // TODO: Will be used when API integration is implemented
...
@Component({
  ...
  template: `
    <div class="bg-gray-50">
      <!-- Hero Banner -->
      <section
        class="relative h-64 bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 overflow-hidden"
      >
        <img
          ngSrc="/assets/images/sony-headphones.png"
          width="1200"
          height="400"
        ...
        <div class="relative mx-auto px-6 h-full flex items-center">
          <div class="text-white">
            ...
            <p class="text-lg text-gray-200 mb-6">
              Track negotiations, respond to offers, and close deals
            </p>
            <button
              class="bg-[#E94C2A] hover:bg-[#FF6B47] text-white px-6 py-3 rounded-lg font-semibold"
        ...
        <div class="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
          ...
          >
            <div class="flex-1 max-w-lg">
              <input
                class="block w-full pl-3 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#E94C2A]"
                type="text"
                placeholder="Search offers by product or user..."
                [(ngModel)]="searchQuery"
              />
            </div>
            <div class="flex flex-wrap gap-3">
              <select
                class="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#E94C2A]"
      ...
      <section class="mx-auto px-6 pb-12">
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="p-6 border-b border-gray-200">
              <h2 class="text-xl font-semibold text-gray-900">
                Offers Sent ({{ filteredSent.length }})
              </h2>
            </div>
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center justify-between mb-2">
                        ...
                        <span
                          ...
                          [ngClass]="statusBadge(card.statusTone)"
                          >{{ card.status }}</span
                        >
                      </div>
                      ...
                      <div class="flex flex-wrap gap-2">
                        <a
                          class="text-xs bg-orange-500 text-white px-3 py-1 rounded hover:bg-orange-600 inline-flex items-center"
                          [routerLink]="[
                            buildPath(
                        ...
                        >
                        <button
                          class="text-xs bg-[#E94C2A] text-white px-3 py-1 rounded hover:bg-[#FF6B47]"
                        >
                          Message
                        </button>
                        <button
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="p-6 border-b border-gray-200">
              <h2 class="text-xl font-semibold text-gray-900">
                Offers Received ({{ filteredReceived.length }})
              </h2>
            </div>
            <div class="p-6 space-y-4">
              <ng-container
                *ngFor="let card of filteredReceived; let i = index"
              >
                <div
                  class="border rounded-lg p-4 hover:shadow-md transition-shadow"
                  [ngClass]="
                    i === 0 ? 'bg-blue-50 border-blue-200' : 'border-gray-200'
                  "
                >
                  <div class="flex items-start gap-4">
                    <img
...
export class OffersComponent implements OnInit {
  ...
  private apiService = inject(ApiService); // TODO: Will be used when API integration is implemented
  ...
  get pendingCount(): number {
    return [...this.sentOffers, ...this.receivedOffers].filter(
      (o) => o.status === 'Pending'
    ).length;
  }
  ...
  createNewOffer(): void {
    this.router.navigate([
      buildPath(ROUTES_ABSOLUTE.APP.OFFERS.ROOT, 'create'),
    ]);
  }
